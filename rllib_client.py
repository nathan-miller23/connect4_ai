...
def _env_creator(env_config):
    ...
    from rllib import ConnectFourMultiAgent 
    return ConnectFourMultiAgent.from_config(env_config)
...
def my_config():
    ...
    ### Environment Params ###
    # How many boards to concatanate for an observation
    buffer_size = 4

    # Number of columsn in board
    columns = 7
    ...
    # Max time for action (0 means no timelimit)
    actTimeout = 0

    # Max time for episode (0 means no time limit)
    runTimeout = 0




    experiment_name = "my_c4_ai"
...
def main(params):
    ...
    results =  { "average_total_reward" : average_episode_reward, "save_paths" : save_paths, "trainers" : trainers }
    return results
